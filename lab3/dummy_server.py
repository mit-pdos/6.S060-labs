...
class DummyServer:
    ...
    def upload_album(self, request):
        if not self._storage.check_token(request.username, request.token):
            return api.UploadAlbumResponse(api.Errcode.INVALID_TOKEN)
        self._storage.update_private_album(request.album.name, request.album)
        return api.UploadAlbumResponse(None)

    def get_album(self, request):
        if not self._storage.check_token(request.username, request.token):
            return api.GetAlbumResponse(api.Errcode.INVALID_TOKEN, None)
        album = self._storage.get_private_album(request.name_album)
        if album == None:
            return api.GetAlbumResponse(api.Errcode.ALBUM_DOES_NOT_EXIST, None)
        return api.GetAlbumResponse(None, album)
...
class DummyStorage:
    ...
    def log_operation(self, username, log_entry):
        self._userbase[username].history.append(log_entry.bytes_data)
    ...
    def update_private_album(self, album_name, album):
        self._private_albums[album_name] = album

    def get_private_album(self, album_name):
...
def encode_contact_book(book):
    ...
    for name in keys:
        ...
        if keys[name][1] == None: # TODO clean up this switch
            ...
            data = keys[name][1].bytes_data
        mappings.append((name, pkey, data))
    if book.metadata() is None:
        return (mappings, None)
    return (mappings, book.metadata().bytes_data)

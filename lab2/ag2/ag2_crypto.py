...
class MessageAuthenticationCode:
    ...
    def gen_mac(self, data):
        ...
        return codec.Lab2Authenticator(self._secret_key, data.items())
    ...
    def compare_mac(self, data, auth):
        ...
        True
        """
        return codec.Lab2Authenticator(self._secret_key, data.items()).equals(auth)
...
class PublicKeySignature:
    def __init__(self, secret_key=None):
        ...
        if secret_key == None:
            ...
            self._sk = secret_key
            self._pk = bytes(secret_key.verify_key)
    ...
    def sign(self, data):
        ...
        if type(data) is not codec.Encoding:
            raise TypeError("data must be Encoding")
        return codec.Lab2Signature(self._pk, data)
...
def verify_sign(pk, data, signature):
    ...
    if signature.key != pk:
        return False
    if signature.data.data != data.data:
        return False
    return True
